...
# returns immediately (deny by default, or approve after explicit config / a
# verified no-host-access sandbox). This separate flag lets those callbacks run
# without reopening a copied gateway/CLI approval surface.
...
def _consume_session_approvals(session_key: str, pattern_keys, source=None) -> None:
    ...
    """
    keys = _coerce_allowlist_patterns(pattern_keys)
    aliases = {
        alias
        for key in keys
...
def _run_approval_gate(
    *,
    ...
    # opt-in approve, or sandbox-gated approve after runtime attestation.
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_gateway or is_ask:
        ...
        if notify_cb is not None:
            ...
            # approval round-trip before this invocation returns.
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    if not interactive_approval_wait_allowed():
        ...
        # immediate auto-approve callbacks. A verified container normally exits
        # through the sandbox guard above; explicit subagent_auto_approve may
        # authorize an unsandboxed child by configuration.
